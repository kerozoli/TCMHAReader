...
def main() -> int:
    ...
    ds_active = sum(1 for ch in data.downstream if ch.frequency or ch.power)
    us_active = sum(1 for ch in data.upstream if ch.frequency or ch.power)

    print(f"Downstream channels: {len(data.downstream)} (active: {ds_active})")
    for ch in data.downstream:
        print(f"  DS {ch.channel_id}: lock={ch.lock_status!r}, {ch.frequency} MHz, {ch.power} dBmV, {ch.snr} dB, BER={ch.ber}%, {ch.modulation}")

    print(f"Upstream channels: {len(data.upstream)} (active: {us_active})")
    for ch in data.upstream:
        print(f"  US {ch.channel_id}: lock={ch.lock_status!r}, {ch.frequency} MHz, {ch.power} dBmV, {ch.modulation}")
